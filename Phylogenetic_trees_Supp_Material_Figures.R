...
ML <- drop.tip(ML, tip = c("Smelophyllum_capense", "Averrhoidium_gardnerianum", "Distichostemon_dodecarpus","Dimocarpus_longanmaliensis","Tina_coursii"))
write.tree(ML,"Phylogenetic_trees/Sapindaceae_RAxML_100_bs_dna_species_tree_USE_THIS_simple.nwk")

#Plot tree
pdf("Phylogenetic_trees/RAxML_radial_clades_Appendix_S2.pdf")
#Ladderize tree
x <- ladderize(ML, FALSE)
ML$node.label[1] <- ""

#Adjust BS to be plotted on tree
#ML$node.label <- as.numeric(ML$node.label)/100
...
nodesClades <- c(grep("Xanthoceras", ML$tip.label), 
                 ...
                 getMRCA(ML, tip=c(which(ML$tip.label == "Cubilia_cubili"), which(ML$tip.label == "Dimocarpus_longan"))),
                 getMRCA(ML, tip=c(which(ML$tip.label == "Tristira_triptera"), which(ML$tip.label == "Eriocoelum_rubiginosum"))),
                 which(ML$tip.label == "Tristiropsis_sp"),
                 ...
                 getMRCA(ML, tip=c(which(ML$tip.label == "Thouinia_acuminata"), which(ML$tip.label == "Allophylus_sp"))),
...
ASTRAL <- drop.tip(ASTRAL, tip = c("Smelophyllum_capense", "Averrhoidium_gardnerianum", "Distichostemon_dodecarpus","Dimocarpus_longanmaliensis","Tina_coursii"))
write.tree(ASTRAL,"Phylogenetic_trees/Sapindaceae_astral_dna_species_tree_bs_less10pc_USE_THIS_simple.nwk")

#Plot tree
pdf("Phylogenetic_trees/ASTRAL_radial_clades_Appendix_S3.pdf")
#Ladderize tree
...
nodesClades <- c(grep("Xanthoceras", ASTRAL$tip.label), 
                 ...
                 getMRCA(ASTRAL, tip=c(which(ASTRAL$tip.label == "Cubilia_cubili"), which(ASTRAL$tip.label == "Dimocarpus_longan"))),
                 getMRCA(ASTRAL, tip=c(which(ASTRAL$tip.label == "Tristira_triptera"), which(ASTRAL$tip.label == "Eriocoelum_rubiginosum"))),
                 which(ASTRAL$tip.label == "Tristiropsis_sp"),
                 ...
                 getMRCA(ASTRAL, tip=c(which(ASTRAL$tip.label == "Thouinia_acuminata"), which(ASTRAL$tip.label == "Allophylus_sp"))),
